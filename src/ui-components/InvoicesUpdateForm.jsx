...
export default function InvoicesUpdateForm(props) {
  const {
    id: idProp,
    invoices,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = invoicesRecord
      ? { ...initialValues, ...invoicesRecord }
      : initialValues;
  ...
  const [invoicesRecord, setInvoicesRecord] = React.useState(invoices);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Invoices, idProp)
        : invoices;
      setInvoicesRecord(record);
    };
    queryData();
  }, [idProp, invoices]);
  React.useEffect(resetStateValues, [invoicesRecord]);
  const validations = {
    invoice_number: [],
    date: [],
    buyer_phone: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Invoices.copyOf(invoicesRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "InvoicesUpdateForm")}
      {...rest}
    >
      <TextField
